...
def simu_partial_align_data(N, int_h, n_knots, sig_warp):
    delta_h = int_h[1] - int_h[0]
    knots_h = np.linspace(int_h[0], int_h[-1], n_knots)
    ...
    for i in range(N):
        ...
        ' [A_i, B_i] '
        delta_i = 0.25*delta_h*np.random.random(1) + 0.6*delta_h
    ...
    for i in range(N):
        ...
        for j in range(i+1,N):
            if i!=j:
                ...
                int_ab_array[i][j] = np.array([knots_ij[0], knots_ij[-2]])
                int_ab_array[j][i] = np.array([knots_ij[0], knots_ij[-2]])
    ...
    sig = np.random.normal(loc=0.0, scale=sig_warp, size=(N,N))
    ...
    for i in range(N):
        for j in range(i+1,N):
            ...
            g_tilde = UnivariateSpline(s_i, np.array([g(t_) for t_ in s_i]), k=4, s=0.001)
            g_tilde_prime = g_tilde.derivative()

            interm_grid_array[i] = s_i
            interm_f_array[i] = f_array[i].evaluate(g_tilde(s_i)).squeeze()*g_tilde_prime(s_i)
    ...
    simu_partial_align = collections.namedtuple('simu_partial_align', ['func_curve', 'final_grid', 'init_grid', 'final_func', 'init_func', 'int_AB', 'int_ab', 'coef_h', 'L'])
    out = simu_partial_align(func_curve, final_grid, grid_array, final_f_array, f_eval_array, int_AB_array, int_ab_array, coef_h, L)
    return out
