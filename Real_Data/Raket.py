...
""" -------------------------------------------------- Construction of data and trajectories list. --------------------------------------------------- """
...
""" --------------------------------------------------------- Load data preprocess if preprocessing already done ---------------------------------------------- """
...
param_model = {"nb_basis" : nb_knots, "domain_range": (0,1)}
...
for i in range(n_cond):
    ...
    out = Parallel(n_jobs=-1)(delayed(global_estimation)(TruePopCondSubjFP[j], param_model, smoothing, hyperparam=hyperparam, opt=True, param_bayopt=param_bayopt, alignment=False)
                                for j in range(n_subj))
...
hyperparam = [0.011, 0.000000001, 0.0000001]
param_bayopt = {"n_splits":  10, "n_calls" : 80, "bounds_h" : (0.015, 0.08), "bounds_lcurv" : (1e-12, 1e-8), "bounds_ltors" : (1e-10, 1e-6)}
nb_knots = 100
param_model = {"nb_basis" : nb_knots, "domain_range": (0,1)}
smoothing = {"flag":False, "method":"karcher_mean"}
...
out = Parallel(n_jobs=-1)(delayed(global_estimation)(array_TruePopFP[i], param_model, smoothing, hyperparam=hyperparam, opt=True, param_bayopt=param_bayopt, alignment=False)
                            for i in range(n_cond))
...
filename = "Raket_data_means_per_cond"
